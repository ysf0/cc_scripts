# Function names should start with c_ prefix.
...
# - c_cassandra_table_print_data ---> prints the rows of given table-name.
# - c_cassandra_execute_cql ---> executed the CQL on remote cassandra.
c_demo_function() {

    ##############################
    # ABOUT
    ##############################
    # This is DEMO function. All notes/comments should be written here (inside ABOUT section).
    ##############################
    ##############################
    ##############################
    ...
    # ___errorHandler prints both last command exit-code and "100999".
    # "100999" is your custom exit-code. you define it unique for each error case.)
    # Out custom-error codes start from 100000. This is not because the lower values are reverved,
    # this is only because we can search them with grep to find directly on cc_script.sh file.
    ___string_ends_with "HELLO WORLD" "ABC" && {
        ___errorHandler "$?" 100999
        return
    }
    ...
    # Always explain what is the parameters like below:
    # -f --> Force delete
    rm -f "/path/to/delete"

    # If your business logic may take time, then put the below lines at the end of your function.
    # c_notify_user will do those for you:
    # 1- a "beep" sound
    # 2- will show a call the OS native notification system
    # 3- will print the message on screen
    # c_notify_user has it's own exit code. Therefore you should always backup $LAST_COMMAND_EXIT_CODE.
    local -r LAST_COMMAND_EXIT_CODE="$?"
    c_notify_user '2' "ping finished"

    # Always return last exit-code of your last executed command. So the end user may see it.
